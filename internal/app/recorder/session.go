...
func decodeMessage(msg *protocol.Message) (eventType string, payloadJSON string, err error) {
	eventType = protocol.TypeName(msg.Type)

	var payload interface{}

	switch msg.Type {
	case protocol.MsgTypeRotation:
		events, err := protocol.DecodeRotation(msg.Payload)
		if err != nil {
	...
	case protocol.MsgTypeBattery:
		event, err := protocol.DecodeBattery(msg.Payload)
		if err != nil {
			return eventType, "", err
		}
		payload = event
	...
	case protocol.MsgTypeCubeType:
		event, err := protocol.DecodeCubeType(msg.Payload)
		if err != nil {
			return eventType, "", err
		}
		payload = event
	...
	case protocol.MsgTypeOrientation:
		event, err := protocol.DecodeOrientation(msg.Payload)
		if err != nil {
			return eventType, "", err
		}
		payload = event
